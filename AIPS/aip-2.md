...
This AIP describes the details of a proposed change to the Ark Number of Votes per Account. In the codebase we inherited the number of votes per account equals the number of forging delegates. However I think that those two numbers should be different as they have different purpose.
...
In this AIP I am trying to be as objective and neutral as possible. The project doesn't exist in a vaccuum so I will make references to observed behaviors, communities and other projects. Because the votes are made by people, I cannot only talk about technicalities in this paper. This paper is by no means a way to criticise or promote projects and/or behaviors but a necessary analysis of observed behaviors.

By learning from existing DPoS project, I have identified multiple recurrent players:
...
The goal of this AIP is to find a Vote Weight that provides the right balance between these entities as I believe they all provide value to the project.
...
I will use LISK blockchain as a study case. There's 101 available votes per account as well as 101 forging delegates. On this project, the community forging has been active for about two month at time of writing.
...
I think that this will have the following effects:
...
Let's narrow down our choices from now, from everything written above, I believe that this number should be above 1 and less than the number of forging delegate. There 51 forging delegates in Ark so:
...
Therefore I propose the Number of Votes per Account to cast to be `10`

I hope that this will give the project the following results:
...
Proposal was rejected for now, at Ark mainnet launch, it will be 1 account = 1 vote
For multiple votes, users will need to split their holding accross multiple accounts